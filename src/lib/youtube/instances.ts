/** Narrow curated fallbacks used only when the share Worker is unavailable. */
...
export function rememberYoutubeBackend(kind: "invidious" | "piped", url: string): void {
  try {
    localStorage.setItem(
      LAST_BACKEND_KEY,
      JSON.stringify({ kind, url: url.replace(/\/+$/, ""), at: Date.now() })
    );
  } catch {
    // ignore
  }
...
/** Curated fallbacks only — no public directory fan-out from the browser. */
export function getYoutubeBackends(): YoutubeBackends {
  return {
    piped: preferLast("piped", PIPED_INSTANCES).slice(0, 2),
    invidious: preferLast("invidious", INVIDIOUS_INSTANCES).slice(0, 2),
  };
}
